...
/// Represents the biological class of the variant. Used locally during parsing
/// to assign a fallback tie-breaker score.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum VariantType { Del, Inv, Dup, Ins, Snp, Replacement }

/// Encapsulates the metrics used to evaluate and resolve colliding variants.
...
/// The core deterministic tie-breaking logic. By implementing `Ord` directly,
/// we allow Rust's native sorting mechanisms to handle resolution gracefully.
impl Ord for Score {
    fn cmp(&self, other: &Self) -> Ordering {
        // TIE-BREAKER HIERARCHY: 
        // 1. Weight (Primary caller score)
        // 2. Genotype Quality (GQ)
        // 3. Allele Frequency (AF)
        // 4. Variant Class (e.g., SVs beat SNPs if tied)
        // 5. Input Index (Bottom variant wins perfect mathematical ties)
...
/// Represents a single VCF record. Designed for "zero-allocation" where possible.
/// We store the raw string and pre-compute tab indices to quickly yield string 
/// slices (&str) rather than creating heavily cloned String objects for columns.
struct Interval {
    line: String,
    tabs: [usize; 9],
    chr: i32,
    ...
    af: f64,
    class_score: i32,
    // Holds the final mutated genotype string for each sample. 
    // If None, the variant survived unharmed and we print the original slice.
    modified_gts: Vec<Option<String>>, 
}

impl Interval {
    /// Parses a raw VCF line into an Interval, determining its physical footprint
    /// and extracting default tie-breaking metrics.
    fn new(line: String, sv_length: i32, n_samples: usize) -> Self {
        // 1. Zero-allocation parsing: track tab boundaries.
        let mut tabs = [0; 9];
        let mut tab_idx = 0;
        ...
        // Helper to extract a specific column as a string slice
        let col = |i: usize| -> &str {
            let start = if i == 0 { 0 } else { tabs[i - 1] + 1 };
            &line[start..tabs[i]]
        };

        // 2. Extract genomic coordinates (Chrom, Pos).
        ...
        // 3. Classify the variant type from the INFO field or REF/ALT lengths.
        ...
        // 4. Calculate the spatial footprint (Length).
        ...
        // Determine physical boundaries for collision detection.
        // Insertions and SNPs act as a 1bp/0bp footprint respectively.
        ...
        // Larger, structurally disruptive variants beat smaller ones in a perfect tie.
    ...
    /// Fetches the entire GT string slice (e.g., "0/1:50:100") for a specific sample.
    ...
    /// Extracts a specific format sub-field (e.g., "GQ", "PS") by index matching.
    ...
    /// Assembles the composite `Score` object used to sort variants.
    ...
    /// Returns just the allele portion ("0|1", "1/1", etc.) of the sample format string.
    ...
    // High-speed genotype logical checks using direct byte indexing on the `gt_bytes` slice.
    ...
    /// Extracts and hashes the Phase Set ("PS") string into a fast u32 comparison integer.
    fn get_ps(&self, sample: usize) -> u32 {
        self.extract_format_field(sample, "PS").map(|s| hash_ps(s)).unwrap_or(0)
    }
}

/// djb2 hash function for converting string phase sets ("PS") into fast integers.
...
/// Fast INFO field extractor without allocating/splitting the entire string into memory.
...
// =====================================================================
// 3. GRAPH SOLVER (GREEDY OCCUPANCY)
// =====================================================================

/// A temporary tracker used during the sorting/claiming phase.
struct Candidate {
    idx: usize,
    score: Score,
}

/// Processes a bounded spatial window of overlapping variants, resolving collisions
/// and mutating genotypes for the "losers".
fn process_window<W: Write>(
    ...
    // We solve collisions on a per-sample basis, as different samples have different genotypes.
    for sample in 0..n_samples {
        let mut candidates = Vec::new();
        
        // Step 1: Filter out reference/missing variants and build candidate list.
        ...
        // Step 2: Rank variants globally. Highest score claims space first.
        candidates.sort_by(|a, b| b.score.cmp(&a.score));

        // Step 3: Identify the "Anchor Phase Set"
        // We look for the PS of the highest-scoring phased variant in the window.
        // Other phased variants must match this PS to be considered safely "Aligned".
        let mut anchor_ps = 0;
        for c in &candidates {
            let iv = &window[c.idx];
        ...
        // Trackers to record claimed physical space on Haplotype 1 and Haplotype 2
        let mut hap1_occupied: Vec<(i32, i32)> = Vec::new();
        let mut hap2_occupied: Vec<(i32, i32)> = Vec::new();

        // Helper to check if a desired spatial block is already taken
        let overlaps = |occupied: &[(i32, i32)], s: i32, e: i32| -> bool {
            occupied.iter().any(|&(os, oe)| std::cmp::max(s, os) <= std::cmp::min(e, oe))
        };

        // Step 4: Greedy Space Claiming
        for c in &candidates {
            let iv = &mut window[c.idx];
            let start = iv.start;
            let end = iv.end;
            ...
            // Check if the space this variant wants is blocked by a higher-scoring variant
            ...
            if is_aligned {
                // Case A: Aligned variants safely claim their specific requested strands.
                keep_h1 = has_alt_h1 && !hit1;
                keep_h2 = has_alt_h2 && !hit2;
                
                if keep_h1 { hap1_occupied.push((start, end)); }
                if keep_h2 { hap2_occupied.push((start, end)); }
            } else {
                // Case B: Unaligned variants (Unphased or PS mismatch).
                // They cannot trust their strand assignment. They must aggressively block 
                // BOTH strands to avoid silent physical collisions.
                if has_alt_h1 && has_alt_h2 {
                    // 1/1 (Homozygous unaligned): Needs both strands, but degrades safely if one is blocked.
                    keep_h1 = !hit1;
                    keep_h2 = !hit2;
                    if keep_h1 { hap1_occupied.push((start, end)); }
                    if keep_h2 { hap2_occupied.push((start, end)); }
                } else if has_alt_h1 || has_alt_h2 {
                    // 0/1 (Heterozygous unaligned): MUST have both free to place safely!
                    if !hit1 && !hit2 {
                        keep_h1 = has_alt_h1;
                        keep_h2 = has_alt_h2;
                        hap1_occupied.push((start, end));
                        hap2_occupied.push((start, end));
                    }
                }
            }

            // Step 5: Apply Genotype Mutations for "Losers"
            // If the variant was blocked from claiming its necessary space, mutate the ALT to REF ('0').
            if (has_alt_h1 && !keep_h1) || (has_alt_h2 && !keep_h2) {
                ...
                if iv.is_unphased(sample) {
                    // Unphased variants degrade entirely to 0/0 if they fail to secure safe placement
                    if !keep_h1 && !keep_h2 {
                        new_a = if gt[0] == b'.' { b'.' } else { b'0' };
                        new_b = if gt[2] == b'.' { b'.' } else { b'0' };
                    }
                } else {
                    // Phased variants selectively degrade ONLY the allele that lost
                    if !keep_h1 { new_a = if gt[0] == b'.' { b'.' } else { b'0' }; }
                    if !keep_h2 { new_b = if gt[2] == b'.' { b'.' } else { b'0' }; }
                }

                // If a mutation occurred, construct the new string and save it to the interval buffer
                if new_a != gt[0] || new_b != gt[2] {
                    ...
                    iv.modified_gts[sample] = Some(new_gt);
                }
            }
        }
    }
    ...
    // Stream the data out. If a genotype was mutated, write the new string. 
    // Otherwise, fast-path the original string slice.
    for iv in window {
        ...
        for sample in 0..n_samples {
            ...
            if let Some(ref mod_gt) = iv.modified_gts[sample] {
                ...
                samples_iter.next(); // Consume and discard the original un-mutated string slice
            } else {
...
fn main() -> io::Result<()> {
    ...
    if clean_args.len() < 4 {
        eprintln!("Usage: {} [--sv-length <int>] <method> <weight_tag> <weight_loc> <default_w> < input.vcf > output.vcf", args[0]);
        return Ok(());
    }

    // Safely parse CLI arguments (ignoring legacy `method` if provided appropriately)
    ...
    // The working memory window. Only holds variants that overlap.
    ...
    while reader.read_line(&mut line)? > 0 {
        ...
        if line.starts_with('#') {
            // Count samples dynamically from the VCF header
            if line.starts_with("#CHROM") { n_samples = line.split('\t').count() - 9; }
            writeln!(output, "{}", line)?;
        } else {
            let iv = Interval::new(line.clone(), sv_length, n_samples);
            
            // WINDOW BOUNDARY LOGIC:
            // Group contiguous intervals together. A new window triggers if the chromosome 
            // changes, or if the new variant starts strictly AFTER the furthest reach of 
            // all previous overlapping variants in the current window.
            if window.is_empty() || (iv.chr == window[0].chr && iv.start <= window_max_end) {
                window_max_end = window_max_end.max(iv.end);
                window.push(iv);
            } else {
                // We've found a clean spatial break. Process the current window, clear it, and start fresh.
                process_window(&mut window, weight_tag, weight_loc, default_w, &mut output, n_samples)?;
                window.clear();
    ...
    // Ensure the final lingering window is processed before shutting down
    process_window(&mut window, weight_tag, weight_loc, default_w, &mut output, n_samples)?;
    output.flush()
}
