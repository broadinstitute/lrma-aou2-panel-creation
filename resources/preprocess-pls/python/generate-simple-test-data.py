...
def generate_sparse_lpl(true_a, true_b):
    """Generates a DRAGEN-compliant sparse LAA and LPL array."""
    alleles = sorted(list(set([0, true_a, true_b])))
    if len(alleles) == 1:
        alleles.append(1)
        
    # LAA only includes the alternate alleles, never the reference (0)
    laa_str = ",".join([str(a) for a in alleles if a != 0])
    ...
    return laa_str, ",".join(map(str, lpls))
...
def main():
    ...
    b5_alts_bubble = ["A" + ("C" * i) for i in range(1, 11)] 
    b6_alts_bubble = ["C", "G", "T", "AA", "AC", "AG", "AT", "CC", "CG", "CT"]

    # Input VCF Shifted/Padded ALTs (Mathematically equivalent)
    b5_alts_input  = ["TA" + ("C" * i) for i in range(1, 11)] 
    b6_alts_input  = [a + "T" for a in b6_alts_bubble]        
    ...
    with open("test_input.vcf", "w") as f:
        ...
        # Changed to LAA
        f.write('##FORMAT=<ID=LAA,Number=.,Type=Integer,Description="Local Alternate Alleles">\n')
        ...
        for i in range(102):
            ...
            snv_id = f"snv_{i}"
            # Changed to LAA
            row = [chrom, str(pos), ".", ref, alt, "50", "PASS", ".", "GT:LAA:LPL"]
            
            for s in range(num_samples):
                true_state = sample_snv_states[snv_id][s]
                # Changed to LAA arrays (no zeros)
                if true_state == 0: row.append("0/0:1:0,20,40")
                elif true_state == 1: row.append("0/1:1:20,0,20")
                else: row.append("1/1:1:40,20,0")
            f.write("\t".join(row) + "\n")
        ...
        for b_name, b_pos, b_ref, b_alts in shifted_bubbles:
            # Changed to LAA
            row = [chrom, str(b_pos), ".", b_ref, ",".join(b_alts), "50", "PASS", ".", "GT:LAA:LPL"]
            for s in range(num_samples):
                a, b = sample_gts[s][b_name]
                laa_str, lpl_str = generate_sparse_lpl(a, b)
                row.append(f"{a}/{b}:{laa_str}:{lpl_str}")
            f.write("\t".join(row) + "\n")
