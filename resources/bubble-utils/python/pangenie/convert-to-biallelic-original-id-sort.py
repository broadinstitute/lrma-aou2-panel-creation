...
# identical to https://github.com/eblerjana/pangenie/blob/master/pipelines/run-from-callset/scripts/convert-to-biallelic.py,
# except 1) with / -> | in GTs, 2) with original IDs in ID field, and 3) sorted by (POS, REF, ALT)
...
# chromosome ->  assigned ID -> [start, REF, ALT, original ID] per chromosome; a single original ID is shared by all records originally split from a multiallelic DeepVariant/GLnexus record
...
for line in gzip.open(args.vcf, 'rt'):
	...
	assert len(ids) == 1
	original_ids = fields[2].split(',')
	assert len(original_ids) == 1
	chrom_to_variants[fields[0]][ids[0]] = [fields[1], fields[3], fields[4], original_ids[0]]
...
for line in sys.stdin:
	...
	# sort the ids by the starting coordinate, then REF, then ALT (to ensure the VCF is stably sorted)
	ids = list(ids)
	ids.sort(key=lambda x : (x[1], chrom_to_variants[fields[0]][x[0]][1], chrom_to_variants[fields[0]][x[0]][2]))
	# create a single, biallelic VCF record for each ID
	for (var_id, coord) in ids:
		vcf_line = fields[:9]
		# set start coordinate
		vcf_line[1] = str(coord)
		# add original ID to ID column of the VCF
		vcf_line[2] = chrom_to_variants[fields[0]][var_id][3]
		# set REF
