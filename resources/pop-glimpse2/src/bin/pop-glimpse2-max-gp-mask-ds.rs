    ...
    for (assigned_id, coord) in sorted_atomic_vars {
        ...
        for s in 0..num_samples {
            ...
            if fmt_out.contains(&"DS") {
                if has_ds {
                    let final_ds = p0 + p1;
                    let rounded = (final_ds * 1000.0).round() / 1000.0;
                    s_out.push(format!("{}", rounded));
                } else {
            ...
            if fmt_out.contains(&"GP") || fmt_out.contains(&"GQ") {
                if has_ds {
                    ...
                    if fmt_out.contains(&"GP") {
                        // Integer-based Largest Bucket algorithm to ensure the rounded percentages perfectly sum to 1.00
                        let mut v0 = (gp0_raw * 100.0).round() as i32;
                        let mut v1 = (gp1_raw * 100.0).round() as i32;
                        let mut v2 = (gp2_raw * 100.0).round() as i32;
                        
                        let diff = 100 - (v0 + v1 + v2);
                        if diff != 0 {
                            if v0 >= v1 && v0 >= v2 {
                                v0 += diff;
                            } else if v1 >= v0 && v1 >= v2 {
                        ...
                        s_out.push(format!("{:.2},{:.2},{:.2}", v0 as f32 / 100.0, v1 as f32 / 100.0, v2 as f32 / 100.0));
                    }
                    if fmt_out.contains(&"GQ") {
                        // GQ remains based on the pure raw probabilities for maximum precision
