...
workflow StatisticalPhasing {
    ...
    input {
        ...
        String chunk_extra_args = "--thread $(nproc) --sequential --uniform-number-variants --window-count 50000 --buffer-count 1000 --window-mb 2 --buffer-mb 0.2 --window-cm 2 --buffer-cm 0.2 --sparse-maf 0"
...
task FilterAndConcatVcfs {
    ...
    command <<<
        ...
        bcftools +setGT -r ~{region} ~{sv_vcf} --no-version -Ou -- -t . -n 0p | \
            bcftools +fill-tags --no-version -Ou -- -t AF,AC,AN | \
    ...
    output {
        ...
        File filter_and_concat_vcf_idx = "~{output_prefix}.vcf.gz.tbi"
    }
...
task CreateShapeitChunks {
    input {
        ...
        String region
        File genetic_map
        String extra_args = "--thread $(nproc) --sequential --uniform-number-variants --window-count 50000 --buffer-count 1000 --window-mb 2 --buffer-mb 0.2 --window-cm 2 --buffer-cm 0.2 --sparse-maf 0"
...
task LigateVcfs {
    ...
    command <<<
        ...
        bcftools +fill-tags --no-version ~{output_prefix}.ligate.bcf \
