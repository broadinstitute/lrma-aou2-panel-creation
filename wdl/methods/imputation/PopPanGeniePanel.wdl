...
workflow PopPanGeniePanel {
    input {
        Array[File] panel_vcf_gzs               # per-chromosome, multiallelic bubble
    ...
    scatter (i in range(length(panel_vcf_gzs)))
    {
        scatter (j in range(length(chromosomes))) {
            call PopPanGeniePanel as ChromosomePopPanGeniePanel {
                input:
                    panel_vcf_gz = panel_vcf_gzs[j],
                    panel_vcf_gz_tbi = panel_vcf_gz_tbis[j],
        ...
        call ConcatVcfs {
            input:
                vcf_gzs = ChromosomePopPanGeniePanel.popped_vcf_gz,
                vcf_gz_tbis = ChromosomePopPanGeniePanel.popped_vcf_gz_tbi,
...
task PopPanGeniePanel {
    ...
    Int disk_gb = 3 * ceil(size(panel_vcf_gz, "GB"))

    command <<<
        set -euox pipefail

        bcftools view ~{panel_vcf_gz} | \
