...
task GLIMPSE2Ligate {
    ...
    command <<<
        ...
        if [ ~{length(phased_vcfs)} -eq 0 ]; then
            echo "ERROR: ligate received no phased BCFs." >&2
            exit 1
        fi
        if [ ~{length(phased_vcfs)} -ne ~{length(phased_vcf_idxs)} ]; then
            echo "ERROR: ligate received ~{length(phased_vcfs)} BCFs but ~{length(phased_vcf_idxs)} indexes." >&2
            exit 1
        fi

        # A File and its index can localize into different Cromwell directories. Give htslib
        # an explicit adjacent BCF/CSI pair for every shard without copying the large BCFs.
        BCFS_FILE=~{write_lines(phased_vcfs)}
        IDXS_FILE=~{write_lines(phased_vcf_idxs)}
        paste "$BCFS_FILE" "$IDXS_FILE" > ligate-input-pairs.tsv
        mkdir ligate-input
        : > ligate-inputs.txt
        i=0
        while IFS="$(printf '\t')" read -r BCF CSI; do
            STEM=$(printf 'ligate-input/shard-%04d' "$i")
            ln -s "$BCF" "$STEM.bcf"
            ln -s "$CSI" "$STEM.bcf.csi"
            printf '%s\n' "$STEM.bcf" >> ligate-inputs.txt
            i=$((i + 1))
        done < ligate-input-pairs.tsv
        if [ "$i" -ne ~{length(phased_vcfs)} ]; then
            echo "ERROR: staged $i ligate inputs for ~{length(phased_vcfs)} BCFs." >&2
            exit 1
        fi

        /bin/GLIMPSE2_ligate --input ligate-inputs.txt --output ~{output_prefix}.bcf --thread ~{ligate_threads}
