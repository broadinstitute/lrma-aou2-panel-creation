...
class HaplotypeTable:
    ...
    def __init__(self, samples, ploidy):
        """
        ploidy: int
    ...
    def merge(self, reference):
        ...
        hap_to_sequence = {}
        hap_to_id = {}
        for hap, columns in hap_to_column.items():
            prev_end = 0
            ...
            if not undefined_allele:
                ...
                for row_index, allele in enumerate(hap.get_alleles()):
                    assert allele != -1
                    if allele > 0:
                        allele_obj = self._alleles[row_index][allele-1]
                        start = allele_obj.start()
                        if prev_end > start:
                            sys.stderr.write('Two overlapping variants at same haplotype at ' + self._chrom + ':' + str(self._start) + ', set allele to missing.\n')
                            sys.stderr.write('ALT ID: ' + allele_obj.id() + '\n')
                            sys.stderr.write('SAMPLES: ' + str([samples[h // ploidy] for h in columns]) + '\n')
